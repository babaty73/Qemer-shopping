...
export async function createOrder(req, res, next) {
  try {
    const { fullName, phone, email, address, telegramUsername, paymentMethod, notes, items } = req.body;

    if (!fullName || !phone || !email || !address || !telegramUsername || !paymentMethod || !items) {
    ...
    const order = await Order.create({
      customer: { fullName, phone, email, address, telegramUsername: normalizeTelegramUsername(telegramUsername) },
      paymentMethod,
